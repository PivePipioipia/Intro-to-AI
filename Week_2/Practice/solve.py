...
class Solution():
    ...
    def draw_legend(self):
        ...
        description = "Each node (m, c, s) represents a state where 'm' is the number of\n missionaries, 'c' the cannibals \n and 's' the side of the boat\n" \
                      "where '1' represents the left side \n and '0' the right side\n\nOur objective is to reach goal state (0, 0, 0)\n" \
                      "from start state (3, 3, 1) by some operators [(0, 1), (0, 2), (1, 0), (1, 1), (2, 0),]\n\n" \
                      "Each tuples (x, y) inside operators represents the number of missionaries and\n" \
                      "cannibals to be moved from left to right if c == 1 and viceversa"
    ...
    def show_solution(self):
        ...
        for i, ((number_missionaries, number_cannnibals, side), node) in enumerate(zip(steps[1:], nodes[1:])):
            ...
            print(f"Step {i + 1}: Move {number_missionaries} missionaries and {number_cannnibals} \
                    cannibals from {self.boat_side[side]} side to {self.boat_side[int(not side)]}.")
